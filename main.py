...
def load_kernel(args):
    ...
    if args.kernel == "average":
        ...
        binomial_1d = np.array([1, 8, 28, 56, 70, 56, 28, 8, 1])
        kernel = np.outer(binomial_1d, binomial_1d)
    ...
    for c in range(I_pad.shape[2]):
        for n in range(I_out.shape[0]):
            for m in range(I_out.shape[1]):
                patch = I_pad[n:n + K, m:m + K, c]
                I_out[n, m, c] = np.sum(patch * h)
    ...
    for c in range(I.shape[2]):
        for u in range(I.shape[0]):
            for v in range(I.shape[1]):
                ...
                I_out_real[u, v, c] = np.sum(I[:, :, c] * np.cos(2 * np.pi * matAB))
                I_out_imaginary[u, v, c] = -np.sum(I[:, :, c] * np.sin(2 * np.pi * matAB))
...
def Recover_function(args, I, u_target_freq, v_target_freq):
    ...
    # Initialize values 
    amplitude_spectrum, phase_spectrum = DFT(args, I)
    height, width = I.shape[0], I.shape[1]
    filtered_amplitude = np.zeros_like(amplitude_spectrum)
    filtered_phase = np.zeros_like(phase_spectrum)

    # Find both the positive and negative frequency coordinates
    pos_u, pos_v = u_target_freq % height, v_target_freq % width
    neg_u, neg_v = (-u_target_freq) % height, (-v_target_freq) % width

    # Keep only those frequencies from the amplitude and phase
    filtered_amplitude[pos_u, pos_v, :] = amplitude_spectrum[pos_u, pos_v, :]
    filtered_phase[pos_u, pos_v, :] = phase_spectrum[pos_u, pos_v, :]
    filtered_amplitude[neg_u, neg_v, :] = amplitude_spectrum[neg_u, neg_v, :]
    filtered_phase[neg_u, neg_v, :] = phase_spectrum[neg_u, neg_v, :]

    # Reconstruct the image via inverse DFT
    I_out = IDFT(args, filtered_amplitude, filtered_phase)
